...
class SkyBlockStructure extends Populator{
	...
	public static function placeObject(ChunkManager $level, $chunk, $Xofchunk, $Zofchunk){
		...
		$level->setBlockIdAt($vec->x + 8, 68, $vec->z + 7, Block::WATER);
		$level->setBlockIdAt($vec->x + 8, 68, $vec->z + 6, Block::WATER);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 5, Block::AIR);
		$level->setBlockIdAt($vec->x + 8, 69, $vec->z + 10, Block::REEDS);
		$level->setBlockIdAt($vec->x + 10, 68, $vec->z + 8, Block::OBSIDIAN);
		$level->setBlockIdAt($vec->x + 10, 68, $vec->z + 9, Block::OBSIDIAN);
		$level->setBlockIdAt($vec->x + 10, 68, $vec->z + 7, Block::OBSIDIAN);
		$level->setBlockIdAt($vec->x + 10, 68, $vec->z + 6, Block::OBSIDIAN);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 8, Block::GOLD_BLOCK);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 9, Block::COBBLESTONE);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 7, Block::DIAMOND_BLOCK);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 6, Block::STONE);
		$level->setBlockIdAt($vec->x + 4, 69, $vec->z + 7, Block::CRAFTING_TABLE);
		$level->setBlockIdAt($vec->x + 4, 69, $vec->z + 8, Block::FURNACE);
		$level->setBlockIdAt($vec->x + 5, 70, $vec->z + 10, Block::HAY_BALE);
		...
		$level->setBlockIdAt($vec->x + 6, 69, $vec->z + 10, Block::LIT_PUMPKIN);
		$level->setBlockIdAt($vec->x + 7, 71, $vec->z + 8, Block::LOG); // 5
		...
		$level->setBlockIdAt($vec->x + 10, 67, $vec->z + 8, Block::DIRT);
		$level->setBlockIdAt($vec->x + 10, 67, $vec->z + 9, Block::DIRT);
		$level->setBlockIdAt($vec->x + 10, 67, $vec->z + 6, Block::DIRT);
		$level->setBlockIdAt($vec->x + 10, 67, $vec->z + 5, Block::DIRT);
	}
